...
class practice_test_reports extends Admin_Controller
{
	...
	{  
		...
		foreach($user_reports as $key => $value){
			$subjects = $this->certificate_model->get_create_test_subjects($value['test_id']);
			$user_reports[$key]['subject_name'] = $subjects;
			// $user_reports[$key]['chapter_name'] = "chapter";
		}
	...
	public function export()
	{
		...
		$fields = 'c.id,concat(u.first_name," ",u.last_name) as user_name,c.test_type,co.name as course_name,cl.name as class_name,c.start_date as start_date,c.id as test_id,c.status as exam_status,exam_code,result,SUM(td.is_correct) as accuracy,COUNT(td.question_id) as questions'; 
		...
		$header_arr = array('Test Type', 'User Name','Attended Date','Course Plan','Class','Subject', 'Questions', 'Accuracy');
		...
		{
			$line = '';
			$single_row = array($row["test_type"]==1?"Create Test":"Standard", $row['user_name'], date("d-m-Y",strtotime($row["start_date"])), $row["course_name"]!=""?$row["course_name"]:"N/A", 
								$row["class_name"]!=""?$row["class_name"]:"N/A", $row["subject_name"]!=""?$row["subject_name"]:"N/A", $row["questions"]!=""?$row["questions"]:"N/A",
