    ...
    */
    ...
    public function get_materials($course="0")
	 ...
	 public function get_meeting()
	 {
		...
		$user_info = $this->session->userdata('user_is_logged_in', 'user_id');
		$where[] = array( TRUE, "id =",$user_info['user_id']);	
		$user_details = $this->base_model->get_advance_list('users', '', "", $where, 'row_array');
		$class_id = $user_details['class_id'];
		if($class_id){
		   $fields = 'meeting_id'; 
		   $where1[] = array( TRUE, "class_id =", $class_id);	
		   $meetingIds = $this->base_model->get_advance_list('meeting_by_class', '*', $fields, $where1, '');
		   $array = array_column($meetingIds, 'meeting_id');
		    ...
		    if( count($array) != 0 ){
				...
				$this->db->where ("(meeting_expiry >=now())");
				$this->db->where('status',1);
				$this->db->order_by('ID', 'desc');
				$result = $this->db->get();
				$data = $result->result_array();
