	...
	<div class="wrapper">
		<ul>			
			...
			<li class="course<?php echo $i;?> course-list-page dyn-course<?php echo $course['id'];?> <?php echo($course['id'] == $course_id && !in_array($course['id'],$check_purchased)?'selected':''); ?>">
				...
				<a href="#" title="Show More" class="paymentshowmore showmore show<?php echo $course['id'];?>">Show More</a>
				<div class="payment-amount">
					<span class="course-amount"><?php echo $currency.number_format($course['price'],2); ?></span>
		...
		<div class="paynow-button">
			<?php echo form_submit('submit','PAY NOW','id="payment_submit"');?>			
		</div>
	</div>
	<?php echo form_close();?>	
</div>
<script>
	$(function(){
		$('#payment_chart').on('submit', function(e){
