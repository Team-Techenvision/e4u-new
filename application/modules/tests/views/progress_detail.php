<?php if($submit_status == 0){ ?>
	<section class="test-overview">
	   ...
	   <?php if(count($selected_subject) >= 1){
	     	foreach($selected_subject as $subj){
	     	?>
	       <a href="#" class="sub_wise_qns exam-details" data-subj_id="<?php echo $subj['subject_id'];?>" style="border-bottom: 4px solid #fd6500;">
	       	<div class="" <?php if($subj['subject_id']== $subject_name['id']){?>  <?php } ?>>
			    <label><?php echo $subj['subject_name'];?></label>
				<h3><?php //echo implode(',',$subj['chapters_name']);
			  echo count($subj['chapters_name']);
			  if(count($subj['chapters_name'])==1){echo " Chapter";}else{echo " Chapters";}
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12">
							   ...
							   <div class="col-md-12 col-sm-12 question-numbers">
							       <ul>
							       	<?php for($i=1;$i <= $count;$i++){
							       			...
							       			$href= base_url().'tests/progress_detail/'.$test_random_id.'/'.($i-1);
							       	?>
							       	<?php 
				...
				<div class="question-section questions-images">
					<h5><b><?php echo ucfirst($subject_name['name']);?></b></h5>
					<?php 
					$attributes = array('id'=>'surprise-test');
					echo form_open_multipart('tests/submit_progress_test',$attributes);?>
					<?php if($count == 0) { ?>
					...
					<h4>Q <?php echo $serial_number['serial_number']; ?></h4>
					 ...
					 <div class="button-section" style="display:<?php if($count == 0){?> none;<?php } else { ?>block;<?php } ?>">
						...
						<span class="ans_prev" <?php if($uri==0){?>style="display: none;"<?php } else { ?>style="display: inline-block;"<?php } ?>><a href="<?php echo base_url();?>tests/progress_detail/<?php echo $test_random_id;?>/<?php echo $previous;?>" title="Previous"><button type="button" class="btn btn-submit">< Previous</button></a></span>
						
						<span class="ans_next"  <?php if($serial_number['serial_number']==$count){?>style="display: none;"<?php } else { ?>style="display: inline-block;"<?php } ?> >
						<a href="<?php echo base_url();?>tests/progress_detail/<?php echo $test_random_id;?>/<?php echo $next;?>"title="Next"><button type="button" class="btn btn-skip ">Next ></button></a></span>
						...
						<?php if($answered==$count){ 
							$submit = "Finish"; ?>
							<span>
							<?php echo form_submit('submit',$submit,'id="progress_test_submit" class="btn btn-submit" title="'.$submit.'"');?>
							</span>
							<?php }else{
							$submit = "Submit";
						?>
...
$("#ModalYes").on('click', function(){
	...
	$(".options").click(function(){
		        	...
		        	if(result != ''){
		        		 result=$.trim(result);
		            		...
		            		if($("#questions_count").val()==parseInt($("#answered_count").val())+1 && $('#serial_no').val()==$("#questions_count").val()){
								if(!$("#progress_test_submit").is(":visible")){
									$(".ans_next").after('<span><input type="submit" title="Finish" id="progress_test_submit"  class="btn btn-submit" value="Finish" name="submit"></span>');
									$(".not_answered").remove();
									$("#submitBtnSta").remove();
								}
		            		}	
		...
		$(document).ready(function(){
			$('.sub_wise_qns').click(function(){
				var sub = $(this).data('subj_id');
				var test_id = $('#test_id').val();
				var test_random_id = "<?php echo $test_random_id;?>";
				var url = base_url+'tests/get_sub_wise_qns';
				$.ajax({
					type: "POST",
					url: url,
					data: "sub_id="+sub+"&test_id="+test_id,
					success: function (result) {
						if(result != ''){
							result=$.trim(result);
								var obj = jQuery.parseJSON(result);
								var href= base_url+'tests/progress_detail/'+test_random_id+'/'+(obj.serial_number-1);
	...
	<?php } else {
		$this->load->view('progress_test_result');
	} ?>
